    ...
    <header className="md:hidden flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 dark:border-slate-800 dark:bg-slate-950 sticky top-0 z-50">
      ...
      {isOpen && (
        <div id="mobile-navigation" onKeyDown={event => {
          ...
          }} className="absolute left-0 top-16 h-[calc(100dvh-4rem)] overflow-y-auto w-full bg-white p-4 dark:bg-slate-950 animate-in slide-in-from-top-5">
            <nav aria-label="Navegação móvel" className="space-y-2">
                {[{ to: "/dashboard", label: "Dashboard", icon: LayoutDashboard },
                  ...
                  <NavLink key={to} to={to} onClick={() => setIsOpen(false)}
                    className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium ${isActive ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-100"}`}>
                    <Icon size={18} />{label}
                  </NavLink>
                ))}
            </nav>
            ...
            <div className="absolute bottom-4 left-4 right-4 border-t pt-4">
                ...
                <Button variant="destructive" className="w-full" onClick={() => { signOut(); navigate("/login"); }}>
